    ...
    if (msg->ct_ipv6) {
        ...
        if(reply){
            ...
            if(  get_16aligned_be32(&l3->ip_src) != msg->ct_key.ipv4.ipv4_dst){
                VLOG_DBG("have a nat, chnage src "IP_FMT" to "IP_FMT, IP_ARGS( get_16aligned_be32(&l3->ip_src)), IP_ARGS(msg->ct_key.ipv4.ipv4_dst));
            }
            if(  get_16aligned_be32(&l3->ip_dst) != msg->ct_key.ipv4.ipv4_src){
                VLOG_DBG("have a nat, chnage dst "IP_FMT" to "IP_FMT, IP_ARGS( get_16aligned_be32(&l3->ip_dst)), IP_ARGS(msg->ct_key.ipv4.ipv4_src));
            }
            ...
            if(  get_16aligned_be32(&l3->ip_src) != msg->ct_key.ipv4.ipv4_src){
                VLOG_DBG("have a nat, chnage src "IP_FMT" from "IP_FMT, IP_ARGS( get_16aligned_be32(&l3->ip_src)), IP_ARGS(msg->ct_key.ipv4.ipv4_src));
            }
            if(  get_16aligned_be32(&l3->ip_dst) != msg->ct_key.ipv4.ipv4_dst){
                VLOG_DBG("have a nat, chnage dst "IP_FMT" from "IP_FMT, IP_ARGS( get_16aligned_be32(&l3->ip_dst)), IP_ARGS(msg->ct_key.ipv4.ipv4_dst));
            }
...
conntrack_add_ct_off_item(struct conntrack *ct,
                               ...
                               struct dp_packet *packet)
{
    if(ctx->conn &&( (packet->md.ct_state & CS_ESTABLISHED) || ctx->conn->nat_info)){
        ...
        if (ctx->reply && !(ctx->conn->offload_flags & CT_OFF_REP)) {
            ...
            } else if (!ctx->reply && !(ctx->conn->offload_flags & CT_OFF_INIT)) {
                conntrack_off_put_flow(ct, ctx, packet, ctx->reply);
